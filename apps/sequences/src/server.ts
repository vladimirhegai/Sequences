  ...
  const readBody = (req: http.IncomingMessage): Promise<string> =>
    new Promise((resolve, reject) => {
      ...
      const onData = (chunk: Buffer) => {
        total += chunk.length;
        // 16MB cap: generous headroom for inline SVG asset uploads.
        if (total > 16_000_000) {
          failed = true;
          chunks.length = 0;
          req.off("data", onData);
  ...
  const server = http.createServer(async (req, res) => {
    ...
    try {
      ...
      if (route === "POST /api/agent/plan") {
              ...
              errors: [
                {
                  path: "provider",
                  message:
                    "no agent provider available — install Codex, Claude Code, or Antigravity CLI (no API key needed) or set an API key",
                },
              ],
            }),
          );
        ...
        void requestPlan(providerId, brief, agentStore.project, runOptions)
          .then(async (result) => {
