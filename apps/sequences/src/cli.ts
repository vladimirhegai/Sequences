 ...
 *   node apps/sequences/src/cli.ts studio examples/sequences/demo-promo
 */
...
async function cmdPlan(dir: string, brief: string | undefined): Promise<void> {
  ...
  if (!providerId) {
    throw new Error(
      "no agent provider available — install/sign into Codex, Claude Code, or Antigravity CLI, or run `cli.ts providers`",
    );
  }
  ...
  printFindings(lintProject(project));
  console.log(`next: node apps/sequences/src/cli.ts studio ${dir}`);
}
