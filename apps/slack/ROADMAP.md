...
| Pre-built asset library (designer-grade parametric assets, spring motion, Asset Lab) | `src/engine/assetContract.ts`, `src/engine/motionSpring.ts`, `src/engine/assets/`, `studio/assetLab.ts` ([ASSETS.md](ASSETS.md)) | asset definitions (typed color/number/text/enum params as root custom props), spring presets + physics, invokable animations, silhouette rhyme families, `asset-<id>` plugin-rail bridge (flag `SLACK_SEQUENCES_ASSETS`, default OFF), operator webview `npm run assets` → 4322 |
